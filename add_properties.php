...
<?php if(isset($_GET['id'])) { ?>
	<?php if ($_SESSION['stype'] == 'Staff' && !in_array('Update', $property_per)) { ?>
	...
	<?php } ?>
<?php } else  { ?>
	<?php if ($_SESSION['stype'] == 'Staff' && !in_array('Write', $property_per)) { ?>
		<?php header('HTTP/1.1 401 Unauthorized'); ?>
		<style>
			.loader-wrapper {
				display: none;
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-sm-12">
						<div class="card">
							<?php if(isset($_GET['id'])) { ?>
								<?php $data = $rstate->query("select * from tbl_property where id=".$_GET['id']."")->fetch_assoc(); ?>
										...
										<div class="row">
											...
											<div class="col-md-4 col-lg-4 col-xs-12 col-sm-12">
												<div class="form-group form-check mb-1">
													<input type="checkbox" class="form-check-input" id="parties-allowed" name="party_allowed" value="1" <?php echo $data['party_allowed'] ? 'checked' : '' ?> />
													<label class="form-check-label" for="parties-allowed">Parties Allowed?</label>
												</div>
												<div class="form-group party-number-field <?php echo $data['party_allowed'] ? '' : 'd-none' ?>">
													<label for="party-cost">Party Cost</label>
													<input type="number" class="form-control" name="party_cost" min="0" value="<?php echo ($data['party_cost'] > 0) ? $data['party_cost'] : '0'; ?>" id="party-cost" />
													<small id="emailHelp" class="form-text text-muted">If free, leave amount as 0.</small>
												</div>
											</div>
											<div class="col-md-4 col-lg-4 col-xs-12 col-sm-12">
												<div class="form-group form-check mb-1">
													<input type="checkbox" class="form-check-input" id="featured-property" name="featured_property" value="1" <?php echo $data['is_featured'] ? 'checked' : '' ?> />
													<label class="form-check-label" for="featured-property">Is Featured Property</label>
												</div>
											</div>
											<div class="col-md-4 col-lg-4 col-xs-12 col-sm-12">
												<div class="form-group form-check mb-1">
													<input type="checkbox" class="form-check-input" id="set-caution-fee" name="set_caution_fee" value="1" <?php echo ($data['caution_fee'] > 0) ? 'checked' : '' ?> />
													<label class="form-check-label" for="set-caution-fee">Set Caution Fee</label>
												</div>
												<div class="form-group caution-fee-field <?php echo ($data['caution_fee'] > 0) ? '' : 'd-none' ?>">
													<label for="party-caution-fee">Caution Fee</label>
													<input type="number" class="form-control" name="caution_fee" value="<?php echo ($data['caution_fee'] > 0) ? $data['caution_fee'] : ''; ?>" min="0" id="caution-fee" />
													<!-- <small id="emailHelp" class="form-text text-muted">If free, leave amount as 0.</small> -->
												</div>
											</div>
										</div>
								...
								<form method="post" enctype="multipart/form-data">
									<div class="card-body">
										<div class="row">
											...
											<div class="col-md-4 col-lg-4 col-xs-12 col-sm-12">
												<div class="form-group form-check mb-1">
													<input type="checkbox" class="form-check-input" id="parties-allowed" value="1" name="party_allowed" />
													<label class="form-check-label" for="parties-allowed">Parties Allowed?</label>
												</div>
												<div class="form-group party-number-field d-none">
													<label for="party-cost">Party Cost</label>
													<input type="number" class="form-control" name="party_cost" min="0" value="0" id="party-cost" />
													<small id="emailHelp" class="form-text text-muted">If free, leave amount as 0.</small>
												</div>
											</div>
											<div class="col-md-4 col-lg-4 col-xs-12 col-sm-12">
												<div class="form-group form-check mb-1">
													<input type="checkbox" class="form-check-input" id="featured-property" value="1" name="featured_property" />
													<label class="form-check-label" for="featured-property">Is Featured Property</label>
												</div>
											</div>
											<div class="col-md-4 col-lg-4 col-xs-12 col-sm-12">
												<div class="form-group form-check mb-1">
													<input type="checkbox" class="form-check-input" id="set-caution-fee" value="1" name="set_caution_fee" />
													<label class="form-check-label" for="set-caution-fee">Set Caution Fee</label>
												</div>
												<div class="form-group caution-fee-field d-none">
													<label for="party-caution-fee">Caution Fee</label>
													<input type="number" class="form-control" name="caution_fee" value="1" min="0" id="caution-fee" />
													<!-- <small id="emailHelp" class="form-text text-muted">If free, leave amount as 0.</small> -->
												</div>
											</div>
										</div>
...
<script>
	$(function(e){
		$(`body`).on(`click`, `#parties-allowed`, function(e){
			$(`.party-number-field`).addClass(`d-none`);
			if ($(this).is(`:checked`)) {
				$(`.party-number-field`).removeClass(`d-none`);
			}
		});

		$(`body`).on(`click`, `#set-caution-fee`, function(e){
			$(`.caution-fee-field`).addClass(`d-none`);
			if ($(this).is(`:checked`)) {
				$(`.caution-fee-field`).removeClass(`d-none`);
			}
		});
	});
</script>
