...
if (isset($_POST["type"])) {
    if ($_POST["type"] == "login") {
        ...
        if ($stype == "Admin") {
            ...
            $count = $h->restatelogin($username, $password, "tbl_staff");
            if ($count != 0) {
                $_SESSION["restatename"] = $username;
                $_SESSION["stype"] = $stype;
                $returnArr = [
        ...
        if ($check == 1) {
            $returnArr = [
                "ResponseCode" => "200",
                "Result" => "true",
                "title" => "Gallery Category Add Successfully!!",
                "message" => "Gallery Category section!",
                "action" => "list_gal_cat.php",
            ];
        }
    } elseif ($_POST["type"] == "edit_gal_category") {
        $property = mysqli_real_escape_string($rstate, $_POST["property"]);
        $title = mysqli_real_escape_string($rstate, $_POST["title"]);
        $status = $_POST["status"];
        $id = $_POST["id"];
        ...
        $table = "tbl_gal_cat";
        $field = ["pid" => $property, "status" => $status, "title" => $title];
        $where = "where id=" . $id . "";
        $h = new Estate();
        $check = $h->restateupdateData($field, $table, $where);
        if ($check == 1) {
            $returnArr = [
                "ResponseCode" => "200",
                "Result" => "true",
                "title" => "Gallery Category Update Successfully!!",
                "message" => "Gallery Category section!",
                "action" => "list_gal_cat.php",
            ];
        }
    ...
    } elseif ($_POST["type"] == "update_reason") {
        $reason = mysqli_real_escape_string($rstate, $_POST["reason"]);

        $id = $_POST["id"];

        $table = "tbl_book";
        $field = ["cancle_reason" => $reason, "book_status" => "Cancelled"];
        $where = "where id=" . $id . "";
        $h = new Estate();
        $check = $h->restateupdateData($field, $table, $where);
        if ($check == 1) {
    ...
    } elseif ($_POST["type"] == "edit_commission") {
        ...
        if ($check == 1) {
            $returnArr = [
                "ResponseCode" => "200",
                "Result" => "true",
                "title" => "Commission Update Successfully!!",
                "message" => "Commission section!",
                "action" => "riderlist.php",
            ];
        }
        ...
        if ($coll_type == "userstatus") {
            ...
            $bdata = $rstate
                ...
                ->fetch_assoc();
            $udata = $rstate
                ->query("select * from tbl_user where id=" . $bdata["uid"] . "")
                ->fetch_assoc();
            $name = $udata["name"];
            $uid = $bdata["uid"];
            ...
            $content = [
                "en" =>
                    ...
                    ", Your Booking #" .
                    $id .
                    " Has Been Check In Successfully.",
            ];
            $heading = [
            ...
            $descriptions =
                "Booking #" . $id . " Has Been Check In Successfully.";
            ...
            $content = [
                "en" =>
                    ...
                    ", Your Booking #" .
                    $id .
                    " Has Been Check Out Successfully.",
            ];
            $heading = [
            ...
            $descriptions =
                "Booking #" . $id . " Has Been Check Out Successfully.";
